...
interface WorkflowStore {
  ...
  zoomIn: () => void;
  zoomOut: () => void;
  createNewGraph: (options?: { confirm?: boolean }) => void;
    ...
    (set, get) => ({
      ...
      executeGraph: async () => {
        ...
        const getDirectedTargetsForValidation = (currentNode: Node) => {
          ...
          try {
            // --- Workflow validation & execution preflight ---
            // We treat the workflow as a directed graph. Before running, we validate that it is a DAG
            // (no cycles), otherwise execution would never terminate.
          ...
          const hasCycle = (nodeId: string): boolean => {
            // DFS cycle detection using an "active recursion stack" (activePath).
          ...
          if (nodes.some((node) => hasCycle(node.id))) {
            alert('Invalid workflow: graph must contain cycles int it. Remove cyclic connections and try again.');
            return;
          }
          ...
          // --- Entry points: triggers start the run ---
          // We start execution from trigger nodes (or nodes whose label includes "start") that are not disabled.
          ...
          while (queue.length > 0) {
            ...
            // --- Halting logic: disabled nodes pause the simulation ---
            // While a node is disabled, the engine accumulates "haltingTime" and waits until it becomes enabled.
      ...
      createNewGraph: (options?: { confirm?: boolean }) => {
        const shouldConfirm = options?.confirm ?? true;
        if (shouldConfirm) {
          if (!window.confirm("Clear all nodes?")) return;
        }
